...
from data.config import username
# Путь к БД
...
def add_userx(user_id, user_login, user_name, balance, all_refill, reg_date):
    with sqlite3.connect(path_to_db) as db:
        db.execute("INSERT INTO storage_users "
                   ...
                   "VALUES (?, ?, ?, ?, ?, ?)",
                   [user_id, user_login, user_name, 10, all_refill, reg_date])
        db.commit()
...
# path_to_db у тебя уже есть в этом файле (скорее всего импортится из config)

def add_login_token(token: str, created_by: int):
    with sqlite3.connect(path_to_db) as db:
        sql = """
            INSERT INTO storage_login_tokens(
                token, created_at, created_by, used, used_by
            ) VALUES (?, ?, ?, 0, NULL)
        """
        db.execute(sql, (token, datetime.datetime.now(), created_by))
        db.commit()


def consume_login_token(token: str, user_id: int) -> bool:
    with sqlite3.connect(path_to_db) as db:
        cur = db.execute("""
            UPDATE storage_login_tokens
               SET used = 1, used_by = ?
             WHERE token = ?
               AND used = 0
        """, (user_id, token))
        ...
        if cur.rowcount == 1:
            db.execute("DELETE FROM storage_login_tokens WHERE token = ?", (token,))
            db.commit()
            return True
...
def add_itemx(category_id, position_id, get_all_items, user_id, user_name):
    with sqlite3.connect(path_to_db) as db:
        for item_data in get_all_items:
            if item_data and not item_data.isspace():
    ...
    with sqlite3.connect(path_to_db) as db:
        ...
        for select_send_item in get_items:
            if send_count != get_count:
                send_count += 1
                save_items.append(f"{send_count}. {select_send_item[2]}")
                sql, parameters = get_format_args("DELETE FROM storage_item WHERE ", {"item_id": select_send_item[1]})
                db.execute(sql, parameters)
                split_len = len(f"{send_count}. {select_send_item[2]}")
            else:
                break
        db.commit()
...
def create_bdx():
    with sqlite3.connect(path_to_db) as db:
        ...
        if len(check_create_users) == 7:
            ...
            print("                        DB не была найдена (1/9) | Создание DB...")
        ...
        if len(check_create_payment) == 6:
            ...
            print("                        DB не была найдена (2/9) | Создание DB...")
        ...
        if len(check_create_settings) == 6:
            ...
            db.execute(sql, parameters)
            print("                        DB не была найдена (3/9) | Создание DB...")
        ...
        if len(check_create_refill) == 10:
            ...
            db.execute("CREATE TABLE storage_refill("
                       ...
                       "dates_unix TEXT)")
            print("                        DB не была найдена (4/9) | Создание DB...")
        ...
        if len(check_create_position) == 8:
            ...
            db.execute("CREATE TABLE storage_position("
                       ...
                       "category_id INTEGER)")
            print("                        DB не была найдена (6/9) | Создание DB...")
        ...
        if len(check_create_item) == 8:
            ...
            db.execute("CREATE TABLE storage_item("
                       ...
                       "add_date TIMESTAMP)")
            print("                        DB не была найдена (7/9) | Создание DB...")
        ...
        if len(check_create_purchases) == 15:
            ...
            print("                        DB не была найдена (8/9) | Создание DB...")

        # Создание БД с хранением одноразовых токенов входа
        check_sql = db.execute("PRAGMA table_info(storage_login_tokens)")
        check_sql = check_sql.fetchall()
        if len(check_sql) == 5:
            print("                        DB была найдена (9/9)")
        else:
            db.execute("""
                CREATE TABLE storage_login_tokens(
            ...
            """)
            print("                        DB не была найдена (9/9) | Создание DB...")
